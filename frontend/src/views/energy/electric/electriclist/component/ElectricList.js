...
const ElectricList = () => {
    ...
    // dataset state
    const [electricListData, setElectricListData] = useState([])
    ...
    useDidMountEffect(() => {
        if (selectBuild && Object.keys(selectBuild).includes('value')) {
            ...
            // 차트에 들어갈 데이터 배열을 받아옴
            getTableData(API_ENERGY_ELECTRIC_TOTAL, param, setElectricListData)
        }// if end
        ...
        <Fragment>
			<Row>
                <div className='d-flex justify-content-start'>
                    <Breadcrumbs 
                        breadCrumbTitle='전력 및 발전량 조회' 
                        breadCrumbParent='에너지관리' 
                        breadCrumbParent2='전력사용관리'
                        breadCrumbActive='전력 및 발전량 조회'
                    />
                </div>
			</Row>
            <Card>
                ...
                <CardBody className='mt-0'>
                    <TotalLabel 
                        num={3}
                        data={electricListData.length}
                    />
                    <div style={{marginBottom: '15px'}}>
                        <DataTable
                            persistTableHead
                            columns={electricListColumns}
                            customStyles={electricListCustomStyles}
                            data={electricListData}
