...
const Disaster = () => {
    ...
    const employeeClass = localStorage.getItem('disasterEmployeeClass')
    ...
    useEffect(() => {
        axios.get(API_REPORT_MANAGE_INSPECTION, {params: {property_id:cookies.get('property').value, start_date: start_date, end_date: end_date, employee_class: employeeClass, type: localStorage.getItem('disasterType')}
        }).then(res => {
            res.data.map((data) => {
