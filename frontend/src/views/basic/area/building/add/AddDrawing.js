...
import { API_BASICINFO_AREA_BUILDING_DRAWING_FORM, API_BASICINFO_AREA_BUILDING_DRAWING_SELECT_ARRAY, ROUTE_BASICINFO_AREA_BUILDING_DRAWING, API_BASICINFO_AREA_BUILDING_DRAWING_DETAIL } from '../../../../../constants'
...
import { axiosPostPut, primaryHeaderColor, sweetAlert } from '../../../../../utility/Utils'
...
const AddDrawing = () => {
	...
	const onSubmit = (data) => {
		...
		if (state.pageType === 'register') {
			formData.append('building', buildingCode[0].value)
			formData.append('type', 'floor_plan')
		}

		const API = state.pageType === 'register' ? API_BASICINFO_AREA_BUILDING_DRAWING_FORM : `${API_BASICINFO_AREA_BUILDING_DRAWING_DETAIL}/${state.id}`  
		
		axiosPostPut(state.pageType, "건물도면", API, formData, setSubmitResult)
	}
	...
	useEffect(() => {
		axios.get(API_BASICINFO_AREA_BUILDING_DRAWING_SELECT_ARRAY, {params: {property_id: cookies.get('property').value, type: 'form', building_id: type}})
		.then(res => {
			setEmployeeClassOptions(res.data.employee_class)
			setFloorOptions(res.data.floor)
			setBuildingCode(res.data.building)
		})
		.catch(res => {
			console.log(API_BASICINFO_AREA_BUILDING_DRAWING_SELECT_ARRAY, res)
		})
	}, [])

	useEffect(() => {
		if (submitResult) {
			navigate(`${ROUTE_BASICINFO_AREA_BUILDING_DRAWING}/${type}`)
		}
	}, [submitResult])

	useEffect(() => {
        if (state.id && employeeClassOptions.length > 1) {
            axios.get(`${API_BASICINFO_AREA_BUILDING_DRAWING_DETAIL}/${state.id}`)
            .then((res) => {
	...
	useEffect(() => {
		...
		console.log("state", state.pageType)
	}, [files, state])
									...
									<Col lg='6' md='6' xs='12'>
										<Row style={{borderTop: '0.5px solid #B9B9C3', borderRight: '0.5px solid #B9B9C3', height: '100%'}}>
											...
											<Col lg='8' md='8' xs='8' className='card_table col text start'>
												<Input disabled value={buildingCode && buildingCode[0].label}></Input>
											</Col>
										</Row>
									</Col>
								</Row>
