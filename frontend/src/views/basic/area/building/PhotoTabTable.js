...
const PhotoDetailTable = (props) => {
	const {columns, tableData, setTabelData, setTableSelect, selectType, detailAPI, type} = props
	const [currentPage, setCurrentPage] = useState(0)
	const navigate = useNavigate()

	const handleRowClick = (row) => {
		if (detailAPI !== undefined) {
			navigate(`${detailAPI}/${type}`, {state:{pageType:'사진', photo_id : row.id}})
		} 
	}
...
export default PhotoDetailTable