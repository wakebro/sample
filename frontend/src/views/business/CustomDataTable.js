...
const CustomDataTable = (props) => {
	const {columns, tableData, setTabelData = null, tableSelect, setTableSelect, selectType, detailAPI, state, rowCnt = 15, footer = [], dispatch} = props
	...
	const handleSort = (column, sortDirection, sortedRows) => {
		...
		sortedRows.sort(function (a, b) {
			return new Date(a[column.sortField]) - new Date(b[column.sortField])
		})
		// desc
		...
		if (setTabelData !== null) {
			dispatch(setTabelData(sortedRows))
			setCurrentPage(0)
		}
	}
	...
	const Footer = ({label, value}) => {
		return (
			<Row className='card_table top' style={{borderTop:'none'}}>
				<Col xs={12}>
					<Row className='card_table table_row'>
						<Col xs={10} className='card_table col col_color text center' style={{borderRight:'none', fontSize: '16px'}}>
							{label}
						</Col>
						<Col xs={2} className='card_table col col_color text center' style={{borderRight:'none', borderLeft:'none', justifyContent:'flex-end', fontSize: '16px'}}>
		...
		<Fragment>
			{
				footer.length !== 0 &&
				footer.map((data, idx) => {
					return (<Footer key={idx} label={data.label} value={data.value}/>)
				})
			}
			<ReactPaginate
				nextLabel=''
