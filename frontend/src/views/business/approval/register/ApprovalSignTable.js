...
import { API_BUSINESS_APPROVAL_REGISTER, ROUTE_BUSINESS_REQUISITION, API_REPORT_APPROVAL_LIST, API_BUSINESS_APPROVAL_DETAIL, API_SYSTEMMGMT_PROPERTY_EMPLOYEE_CLASS } from '../../../../constants'
import Swal from "sweetalert2"
import { axiosPostPutCallback, getTableDataCallback, primaryColor } from '../../../../utility/Utils'

const ApprovalSignTable = (props) => {
    const { 
        setIsOpen, id, userSign, setUserSign, signType, 
        setSignType, inputData, setActiveIndex, prop, navigate,
        orignUserSign, orignSignType, pageType
    } = props
    const cookies = new Cookies()
    const [data, setData] = useState([])
    const [selectTableList, setSelectTableList] = useState([{label: '전체', value:''}])
    const [selectedEmployeeClass, setSelectedEmployeeClass] = useState(selectTableList[0])
    const [searchValue, setSearchValue] = useState('')
    const [showUser, setShowUser] = useState({name:'없음'})
    ...
    const handleNext = () => {
        ...
        if (index === 0) {
            ...
            if (userSign[3] !== '' && userSign[3] !== 0) {
                ...
                const API = pageType === 'register' ? API_BUSINESS_APPROVAL_REGISTER : API_BUSINESS_APPROVAL_DETAIL
                axiosPostPutCallback(pageType, '품의서', API, inputData, function() {
                    navigate(`${ROUTE_BUSINESS_REQUISITION}/${prop}`)
                })
            } else {
                Swal.fire({
                    icon: "info",
...
export default ApprovalSignTable