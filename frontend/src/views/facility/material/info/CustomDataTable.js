...
import { NoData } from "../../../../components/Sentence"
...
const CustomDataTable = (props) => {
	const {columns, tableData, setTabelData, setTableSelect, selectType, toggleModify, setRowId, toggleReplacementModify, detailAPI, setSubId, setSubName, mainImg} = props
	...
	const handleRowClick = (row) => {
		if (detailAPI) {
			if (detailAPI === '대체자재') {
				setRowId(row.id)
				setSubId(row.replacement_material.id)
				setSubName(row.replacement_material.code)
				toggleReplacementModify()
			} else {
				toggleModify()
				setRowId(row.id)
			}
