...
import { ROUTE_STANDARD, API_STANDARD_COSTTYPE, ROUTE_STANDARD_COSTTYPE_DETAIL } from '../../../../constants'
...
const CostType = (props) => {
    ...
    const onSubmit = (data) => {
		...
		const API = pageType === 'register' ? API_STANDARD_COSTTYPE
										: `${API_STANDARD_COSTTYPE}/${state.id}`
	...
	useEffect(() => {
		if (state.type === 'modify') {
			axios.get(`${API_STANDARD_COSTTYPE}/${state.id}`)
			.then(res => {
    ...
    useEffect(() => {
		if (submitResult) {
			if (state.type === 'modify') {
				window.location.href = `${ROUTE_STANDARD_COSTTYPE_DETAIL}/${state.id}`
			} else {
									...
									<Col lg='8' md='8' xs='8' className='card_table col text center'>
										<Row style={{width:'100%'}}>
											...
											<Col lg='4' xs='12'className='card_table col text center border_none' style={{paddingLeft:'0', paddingRight:'0'}}>
												<Row style={{width:'100%'}}>
													<Button size='sm' onClick={() => compareCode(field.value, oldCode, API_STANDARD_COSTTYPE, setCheckCode)}>중복검사</Button>
													{errors.code && <div>&nbsp;</div>}
												</Row>
											</Col>
										</Row>
