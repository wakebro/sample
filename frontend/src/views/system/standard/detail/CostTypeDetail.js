...
import { ROUTE_STANDARD, API_STANDARD_COSTTYPE, ROUTE_STANDARD_ADD } from '../../../../constants' 
...
const DetailFacility = () => {
	...
	const handleDeleteSubmit = () => { // 안쓰는 컴포넌트
		axiosDeleteParm('임대표준', `${API_STANDARD_COSTTYPE}/${id}`, {params:{view_order:data.view_order}}, ROUTE_STANDARD, navigator)
	}

    useEffect(() => {
		axios.get(`${API_STANDARD_COSTTYPE}/${id}`)
		.then(res => {
		...
		<Fragment>
			<Row>
				<div className='d-flex justify-content-start'>
					<Breadcrumbs breadCrumbTitle='임대표준관리' breadCrumbParent='시스템관리' breadCrumbParent2='임대표준관리' breadCrumbActive='인덱스타입 상세' />
				</div>
			</Row>
			<Card>
				<CardHeader>
