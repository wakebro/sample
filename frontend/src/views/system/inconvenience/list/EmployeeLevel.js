...
const FacilityInfo = () => {
	...
	const column = InconvInfoColumn['employee_level']
	const [searchParams, setSearchParams] = useState('')
	const cookies = new Cookies()

    useEffect(() => {
        axios.get(API_INCONV_EMPLOYEE_LEVEL, {params: {property_id:cookies.get('property').value, search: searchParams, select_employee_class: ''}})
        .then(res => {
			...
			<Col>		
				<Card>
					<CardHeader style={{padding: '1.5rem 1.5rem 0rem 1.5rem'}}>
                        <CardTitle tag='h4'>직급 관리</CardTitle>
						<Row>
                            <Button hidden={checkOnlyView(loginAuth, SYSTEM_INCONVENIENCE_EMPLOYEE_LEVEL, 'available_create')}
                                color='primary' 
                                style={{marginLeft: '-22%'}}
                                tag={Link} 
                    ...
                    <CardBody>
                                        ...
                                        <Col className='mb-1' md='6' sm='10'>
                                            <InputGroup>
                                                <Input 
													...
													onKeyDown={e => {
														if (e.key === 'Enter') {
															SearchGetTableData(API_INCONV_EMPLOYEE_LEVEL, { property_id:cookies.get('property').value, search: searchParams, select_employee_class:''}, setData)
														}
													}}
                                                    />
                                                <Button
						...
						<CustomDataTable 
							...
							selectType={false} 
							onRowClicked
							detailAPI={ROUTE_SYSTEMMGMT_INCONV_EMPLOYEE_LEVEL_DETAIL}
							noDataComponent
						/>
                    </CardBody>
				</Card>
