...
import { API_INCONV_CAUSE, ROUTE_INCONV, ROUTE_SYSTEMMGMT_INCONV_CAUSE_DETAIL } from '../../../../constants'
...
const Cause = (props) => {
	...
	const [selectTableList, setSelectTableList] = useState([{ value:'', label: '직종'}])
	...
	const onSubmit = (data) => {
		...
		const API = pageType === 'register' ? API_INCONV_CAUSE
										: `${API_INCONV_CAUSE}/${state.id}`

		axiosPostPut(pageType, "원인유형", API, formData, setSubmitResult)

	}

	useEffect(() => {
		if (state.type === 'register') {
			axios.get(API_INCONV_CAUSE, {params:{property:cookies.get('property').value, search: '', select_employee_class: ''}})
			.then(res => {
				const empClassList = []
			...
			axios.get(`${API_INCONV_CAUSE}/${state.id}`, {params:{property:cookies.get('property').value}})
			.then(res => {
				const empClassList = []
				for (let i = 0; i < res.data.emp_class_list.length; i++) {
					empClassList.push({value:res.data.emp_class_list[i].id, label:res.data.emp_class_list[i].code})
				...
				setSelectTableList(prevList => [...prevList, ...empClassList])
				setDetailData(res.data.cause)
				setOldCode(res.data.cause.code)
			})
		}
    }, [])
	useEffect(() => {
	...
	useEffect(() => {
		if (submitResult) {
			if (state.type === 'modify') {
				window.location.href = `${ROUTE_SYSTEMMGMT_INCONV_CAUSE_DETAIL}/${state.id}`
			} else {
								...
								render={({ field }) => (
									<Col className='card_table col text'>
										<InputGroup>
											...
											<Button style={{borderTopRightRadius: '0.358rem', borderBottomRightRadius: '0.358rem'}} onClick={() => compareCodeWithEmployeeClass(field.value, cookies.get('property').value, oldCode, API_INCONV_CAUSE, setCheckCode, selectEmployeeClass)}>중복검사</Button>
					...
					<Fragment>
						{ state.type === 'modify' &&
							<Button 
								color='report' 
								tag={Link} 
								to={`${ROUTE_SYSTEMMGMT_INCONV_CAUSE_DETAIL}/${state.id}`} 
								state={{
									key: 'cause'
								}} >취소</Button>
						}
