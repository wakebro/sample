...
import { Button, Card, CardBody, CardFooter, Col, Form, FormFeedback, Input, InputGroup, Row } from "reactstrap"
...
const EmployeeClass = (props) => {
	...
	const onSubmit = (data) => {
		...
		const API = (pageType === 'register' ? API_INCONV_EMPLOYEE_CLASS
										: `${API_INCONV_EMPLOYEE_CLASS}/${state.id}`)

        axiosPostPut(pageType, '직종코드', API, formData, setSubmitResult)
    }

	useEffect(() => {
		if (state.type === 'modify') {
			axios.get(`${API_INCONV_EMPLOYEE_CLASS}/${state.id}`, {params:{property:cookies.get('property').value}})
			.then(res => {
	...
	useEffect(() => {
		if (submitResult) {
			if (state.type === 'modify') {
				window.location.href = `${ROUTE_SYSTEMMGMT_INCONV_EMPLOYEE_CLASS_DETAIL}/${state.id}`
			} else {
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Col className='card_table col text'>
                                            <InputGroup>
                                                <Input bsSize='sm' maxLength="32" invalid={errors.code && true} {...field}/>
                                                <Button style={{borderTopRightRadius: '0.358rem', borderBottomRightRadius: '0.358rem'}} onClick={() => compareCodeWithValueProperty(field.value, cookies.get('property').value, oldCode, InconvInfoUrlObj.employee_class, setCheckCode)}>중복검사</Button>
                            ...
                            <Row className='card_table table_row'>
                                ...
                                <Controller
                                    ...
                                    <Col className='card_table col text' style={{flexDirection:'column'}}>
                                        <Input type='number' bsSize='sm' invalid={errors.view_order && true} {...field}/>
                ...
                <CardFooter style={{display : 'flex', justifyContent : 'end', alignItems : 'end', borderTop: '1px solid #dae1e7'}}>
                    {state.type === 'modify' &&
                        <Button color='report' 
                            className="ms-1"
                            tag={Link} 
                            to={`${ROUTE_SYSTEMMGMT_INCONV_EMPLOYEE_CLASS_DETAIL}/${state.id}`} 
                            state={{
                                key: 'normal'
                            }} >취소</Button> 
                    }
                    <Button type='submit' className='ms-1' color='primary'>{state.type === 'register' ? '저장' : '수정'}</Button>
                    <Button className='ms-1' tag={Link} to={ROUTE_SYSTEMMGMT_INCONV_EMPLOYEE_CLASS}>목록</Button>
                </CardFooter>
            </Form>
        </Fragment>
	)
