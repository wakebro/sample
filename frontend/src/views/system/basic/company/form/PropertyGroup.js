...
import { API_SYSTEMMGMT_BASIC_INFO_PROPERTY_GROUP, ROUTE_SYSTEMMGMT_BASIC_COMPANY, ROUTE_SYSTEMMGMT_BASIC_COMPANY_DETAIL } from '../../../../../constants'
...
const PropertyGroup = (props) => {
	...
	// GET
	const getDetail = (id) => {
		axios.get(`${API_SYSTEMMGMT_BASIC_INFO_PROPERTY_GROUP}/${id}`)
		.then(res => {
	...
	const onSubmit = (data) => {
		...
		const API = pageType === 'register' ? `${BasicInfoAPIObj['propertyGroup']}/-1`
										: `${BasicInfoAPIObj['propertyGroup']}/${rowId}`

		axiosPostPut(pageType, BasicInfoLabelObj['propertyGroup'], API, formData, setSubmitResult)
	}

	// DELETE
	const onDelete = () => {
		const API = `${BasicInfoAPIObj['propertyGroup']}/${rowId}`
		axiosDelete(BasicInfoLabelObj['propertyGroup'], API, setSubmitResult)
	}
											...
											render={({ field }) => (
												<Col className='card_table col text start'>
													<InputGroup>
														...
														<Button style={{borderTopRightRadius: '0.358rem', borderBottomRightRadius: '0.358rem'}} size='sm' onClick={() => compareCodeWithValue(field.value, oldCode, API_SYSTEMMGMT_BASIC_INFO_PROPERTY_GROUP, setCheckCode)}>중복검사</Button>
								...
								<Col lg='6' md='6' xs='12' className='card_table top'>
									<Row className='card_table table_row'>
										...
										<Controller
											...
											control={control}
											render={({ field }) => (
												<Col lg='8' md='8' xs='8' className='card_table col text start' style={{flexDirection:'column'}}>
